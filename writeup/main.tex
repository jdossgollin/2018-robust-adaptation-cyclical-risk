...
In 2011 five successive cyclones struck Thailand, causing massive damage to public and private infrastructure and crippling global supply chains.\footnote{See Masa Haraguchi's work}
Later in June 2013 a long rainy period was followed by three summertime cyclones tracking over the same region, leading to massive flooding in central Europe \citep{Grams2014}.
At the same time a multi-year drought in S\~ao Paulo threatened to dry out one of the world's largest cities, and in 2017 the Atlantic Hurricane season led to over \usd{100 billion} in reinsurance loss.\footnote{\href{http://www.artemis.bm/blog/2018/02/27/berkshire-hathaway-can-easily-weather-a-400bn-mega-catastrophe-buffett/}{Click for blog post on \url{artemis.bm}}}
Since classical analysis of climate extremes for use in risk analysis and mitigation studies assumes that  the historical or simulated record of events constitutes an independent and identically distributed process, adaptation systems are overwhelmed by these multi-year droughts or multi-hurricane hits.

Indeed, the scientific literature has recognized that even small nonstationarity (temporal changes in dynamics of the underlying hydroclimatalogical process) can lead to significant trends in the estimated $T$-year return periods or probability of exceedance $p_T$ \citep{Milly2008,Vogel2011,Salas2014,Merz2014,Obeysekera2016}.
...
Despite the wide suite of available models, \emph{the question of how one should use these nonstationarity estimates for the design of risk mitigation infrastructure or the pricing of insurance like instruments has been much less studied}.\footnote{All of these citations are for floods, can we look at other types of extremes? Even droughts would be helpful.}
...
\end{enumerate*}
Because of errors inherent to discretization and parameterization of the laws of motion, bias correction methods are typically applied at each step of the chain; while this typically improves the model chain's representation of the historical record, the extrapolation of these bias corrections far into the future does not inspire confidence \citep{Dankers2009,Ott2013,Merz2014,Dittes2017}.\footnote{Farnham et al 2018!}
...
% PHYSICAL PLAUSABILITY OF TRENDS 
% SPATIAL CASE
% THIS IS RELEVANT FOR OTHER NATURAL HAZARDS -- DISCUSS
...
Implications
\begin{itemize}
  \item Blind use of trend models leads to sub-optimal models \emph{even when there is a trend} for many combination of \(M,N\)
  \item Bias-variance tradeoff of model complexity
  \item Need to consider the structure of the underlying trend, $N$, and $M$ when choosing a model
  \item Should consider multiple models, \eg by ``stacking'' \citep{Yao2017}
\end{itemize}
