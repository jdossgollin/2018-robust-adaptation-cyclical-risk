...
\end{equation}
The parameter \( \mu(t) \) is generated using two different approaches, summarized below, while for both we assume a constant coefficient of variation:
\begin{equation}
...
Alternatively, we consider a 2-state Markov chain with a constant transition probability which is the same for both states, such that the transition matrix is
\begin{equation*}
...
As \cref{sec:method-stationary} except that
\begin{equation*}
  \mu = \mu_0 + \beta_\mu \qty(t - t_0)
\end{equation*}
and so the parameters \( \mu_0, \beta_\mu \) must be estimated rather than just \(\mu\).
...
\subsection{Flood Estimation and Uncertainty\label{sec:methods-uncertainty}}
...
We still need to find a good case study -- it can be a highly simplified one.
We can come up with some numbers and a single streamflow sequence that has been observed.
