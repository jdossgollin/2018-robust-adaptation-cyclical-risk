...
\end{figure}
As shown in \cref{fig:m-n-sketch}, the importance of predicting, and potential for doing so skillfully, depend on the project planning period $M$ and the information available, $N$.
When $M$ and $N$ are short, only a ``rough guess'' using simple models is feasible, and increases in model complexity only add uncertainty.
If $M$ is short and $N$ is long, however, the \gls{lfv} in the system may be skillfully predicted, thereby reducing the cost of such an instrument.
If $N$ remains long but $M$ increases, then informational uncertainty may be reduced, but intrinsic uncertainty will remain, and may be large.
