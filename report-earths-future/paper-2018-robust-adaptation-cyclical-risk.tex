...
Even if \add{future} mitigation efforts are successful, \add{existing levels of atmospheric $\text{CO}_2$ and ocean heat content motivate the development of} novel adaptation strategies\remove{ are needed}.
...
The Hurst phenomenon associated with stochastic long-memory processes and has been invoked to explain long scale fluctuations in geophysics, physics, biology, medicine, traffic, network dynamics as well as for financial time series.
This phenomena can be characterized as:
\begin{equation}\label{eq:hurst}
  \frac{R_n}{S_n} \sim n^H,
\end{equation}
where $H \in [0.5, 1]$ is the Hurst coefficient, $R_n$ is the range of the first $n$ cumulative deviations from the mean, and $S_n$ is the sample standard deviation \citep{Hurst:1951tc,oconnell:2016,klemes:1974}.
\Cref{eq:hurst} measures the tendency of a time series to either regress to the mean or cluster towards the extremes.
The Hurst exponent $H$ is directly related to the fractal dimension of a process, and there is a rich multi-disciplinary literature as to the process level and statistical justification of long memory and fractal processes, especially in hydrology \citep{mandelbrot:1985,mandelbrot:1969,beran:1994}.
...
Based on this theory, regimes and quasi-periodic phenomena with amplitude that increases with recurrence interval would be the norm rather than the exception.
...
If these climate modes vary on decadal or longer time scales, such as the \gls{pdo} \citep{Mantua:1997kj}, then this \gls{lfv} may appear in local hydrologgical records as well, leading the past to be a poor predictor of future behavior.
For example, \citet{Jain:2001hz} demonstrated that in a system with substantial \gls{lfv}, due to \gls{enso}, the past $N$ years can be a poor proxy for the following $N$ years even when no secular change is present.
When records are short and low-frequency external forcing or local memory dominates, it is easy to mistake the Hurst phenomenon for a secular trend which may not continue from the past into the future \citep{bhattacharya:1983}.
t is also difficult to identify the dominant scales of variability \citep{newman:2016,Jin:1994wq}.

We illustrate this type of \gls{lfv} in \cref{fig:observed-lfv}, which shows a \SI{500}{year} drought reconstruction from the \gls{lbda} \citep{Cook:2010bz}, a \SI{100}{year} record of annual maximum streamflowflow on the American River at Folsom, and the global wavelet power spectrum for both \citep{Torrence:1998jp,Roesch:wlBQQoIs}.
...
\add{By contrast, the informational uncertainties for a sequence of short-term instruments are $N, N+M, N+2M, \ldots$, which may lead to improved identifiability and predictability of relevant climate signals.}\remove{rather than a sequence of $N$, $N+\Delta N, \ldots$}.
