...
A critical point for decision-making under uncertainty is that if precise climate information over the next few years is available, then this can significantly alter the choice between a large, long-duration project which may take over a year to construct, and a sequence of smaller, short duration projects that can be executed quickly.
For example, if there is credible information that the risk to be mitigated will be much higher in the next few years, then a more costly project would be justified under traditional \gls{cba}.
However, if the construction period were too long, a financial risk mitigation instrument might be needed in the immediate term to cover potential losses before the large project is constructed.
Conversely, if the near term risk is projected to be low, then deferral of the large, expensive project may be warranted, and smaller or nonstructural alternatives may be preferred.
If the risk changes in a cyclical manner, then these decisions become even more nuanced, but there may be an opportunity for significantly improving adaptation using a portfolio risk management approach.
Thus, the precision with how near term and long term directions of climate risk evolve plays directly into adaptation choice. 
...
For the sake of presentation we motivate our results through a simple flood risk example.
The points we seek to make apply to other hydroclimatic risks, and in particular those which may be easily characterized through a time series of annual maxima or minima of the process.
...
\end{figure}
We also showed that the importance of predicting different climate signals, and potential for doing so skillfully, depend on the project planning period $M$ and the information available, $N$.
...
As $N$ increases, however, the potential for skillful prediction of \gls{lfv} over the short planning period increases, potentially favoring implementation of a flexible or adaptive instrument.
