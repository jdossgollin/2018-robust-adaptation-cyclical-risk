...
\usepackage{
  ...
  trackchanges,       % to track any new text
...
Even if \add{future} mitigation efforts are successful, \add{existing levels of atmospheric $\text{CO}_2$ and ocean heat content mean that} novel adaptation strategies are needed.
...
Although climate risk has been traditionally managed with centrally planned structural instruments (e.g.\@\xspace, a levee), the high price \citep{Papakonstantinou:2016ve}, environmental costs \citep{dugan:2010}, and vulnerability to biased climate projections \citep{lempert:2007} have recently slowed their implementation.
\remove{Instead}\add{Rather}, factors such as New York City have turned to a combination of structural\add{ (e.g.\@\xspace, a stormwater barrier)}, operational\add{ (e.g.\@\xspace, improved evacuation routes)}, and financial\add{ (e.g.\@\xspace, index insurance)} instruments for reducing vulnerability and increasing resilience to climate extremes \citep{CityofNewYork:2013uh}.
...
In this paper we focus on \add{the more narrow question of} how the temporal structure of climate risk, and the uncertainties associated with \add{its estimation}\remove{estimating it}, influence the answers to these questions.
...
Of these processes, \gls{acc} has received the most attention in the climate adaptation literature and its influence on \add{some} river floods, droughts, hurricanes, urban flooding, and many other climate hazards \remove{are}\add{has been the subject of substantial investigation} \remove{well documented} \citep[e.g.\@\xspace,][]{Coumou:2012bc,Milly:2008dg,OGorman:2009hj,Trenberth:2003bj}.
...
The Hurst phenomenon associated with stochastic long-memory processes has been invoked to explain long scale fluctuations in geophysics, physics, biology, medicine, traffic, network dynamics as well as for financial time series:
\begin{equation}
  \frac{R_n}{S_n} \sim n^h,
\end{equation}
where $h \in [0.5, 1]$, $R_n$ is the range of the first $n$ data points, and $S_n$ is the sample standard deviation \citep{Hurst:1951tc,oconnell:2016}.
A variety of theories, including infinite memory, nonstationarity of the process, and specific storage mechanisms have been put forth to explain the Hurst phenomenon, none fully satisfactorily \citep{klemes:1974}.
The Hurst exponent is directly related to the fractal dimension of a process, and there is a rich multi-disciplinary literature as to the process level and statistical justification of long memory and fractal processes, especially in hydrology \citep{mandelbrot:1985,mandelbrot:1969,beran:1994}.
\citet{lovejoy:2012,lovejoy:2013a} and \citet{lovejoy:2013} also argue that multifractal and long memory processes are an appropriate description of climate in general and weather extremes in particular. 
Based on this theory, regimes and quasi-periodic phenomena with increasing amplitude as their recurrence interval increases would be the norm rather than the exception.
Similar arguments are made by \citet{koutsoyiannis:2003}, \citet{markonis:2013}, and \citet{oconnell:2016}, and detailed discussions and some philosophical perspectives are offered by \citet{bloschl:2010} and \citet{montanari:2003}.
Wavelet methods have also been used to estimate the Hurst exponent \citep{simonsen:1998,chamoli:2007}, and to design simulation algorithms that reproduce self-similarity, long range dependence and quasi-periodic regimes \citep{Kwon:2007fj,bullmore:2001,geweke:1983,feng:2005}.

Links between local hydrological processes and large-scale climate patterns, often referred to as teleconnections \citep{Angstrom:1935ej}, have also been used to explain \gls{lfv} in hydroclimate systems.
If these climate modes vary on decadal or longer time scales, such as the Pacific Decadal Oscillation \citep{Mantua:1997kj}, then this \gls{lfv} may appear in local hydrologgical records as well, leading the past to be a poor predictor of future behavior.
For example, \citet{Jain:2001hz} demonstrated that in a system with substantial \gls{lfv}, due, e.g.\@\xspace, to \gls{enso}, the past $N$ years can be a poor proxy for the following $N$ years even when no secular change is present.
When records are short, and whether low-frequency external forcing or local memory dominates, it is easy to mistake the Hurst phenomenon for a secular trend which may not continue from the past into the future \citep{bhattacharya:1983}.
...
In \cref{sec:markov-generating} we consider an alternative parameterization of $\mu(t)$, i.e.\@\xspace an alternative to \cref{eq:nino3}, which considers a Markovian state transition rather than an explicit \gls{enso} model, and note a general agreement of results.
...
\add{We refer tho this model as ``LN2 Linear Trend''}
...
A \gls{hmm} is a latent variable model in which the system being modeled is assumed to follow a Markov process with unobserved (i.e.\@\xspace hidden) states $S(t)$ \citep{Rabiner:1986jk}.
...
For a given choice of $M$, \add{and} $N$, generating model, we compare the synthetic streamflow sequence's $N$-year ``historical record'', and the $K=1000$ posterior simulations of future flows.
...
Projections with each of the three fitting models \add{described in} \cref{sec:methods-estimating} are also shown.
...
These findings were derived \add{conceptually and through} \remove{using simple models} \add{idealized computational experiments} for \add{simulating and predicting climate} risk\emph{ projection}, but the principles are applicable to more complex, physically based methods.
