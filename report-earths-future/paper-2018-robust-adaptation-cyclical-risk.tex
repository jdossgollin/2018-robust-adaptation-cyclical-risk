...
\remove{Instead}\add{Rather}, actors such as New York City have turned to a combination of structural\add{ (\eg, a stormwater barrier)}, operational\add{ (\eg, improved evacuation routes)}, and financial\add{ (\eg, index insurance)} instruments for reducing vulnerability and increasing resilience to climate extremes \citep{CityofNewYork:2013uh}.
...
Of these processes, \gls{acc} has received the most attention in the climate adaptation literature and its influence on \add{some} river floods, droughts, hurricanes, urban flooding, and many other climate hazards \remove{are}\add{has been the subject of substantial investigation} \remove{well documented} \citep[\eg,][]{Coumou:2012bc,Milly:2008dg,OGorman:2009hj,Trenberth:2003bj}.
...
\remove{Permanent or secular change is not, however, a prerequisite for the inadequacy of relying on historical records.}
\add{Secular change is not the only mechanism which can cause historical records to provide a biased view of future risk.}
\annote{One well-studied example}{This whole paragraph is added, but the track changes packages doens't allow math or references.} in the hydrological literature is the Hurst phenomenon, $\frac{R_n}{S_n} \sim n^h$, where $h \in [0.5, 1]$, $R_n$ is the range of the first $n$ data points, and $S_n$ is the sample standard deviation \citep{Hurst:1951tc,oconnell:2016}.
A variety of theories, including infinite memory, nonstationarity of the process, and specific storage mechanisms have been put forth to explain the Hurst phenomenon, none satisfactorily \citep{klemes:1974}.
