...
class SyntheticFloodSequence(BaseSequence):
    ...
    def _from_file(self) -> Tuple[xr.DataArray, bool]:
        ...
        try:
            ...
            if (attr_desired == attr_observed) and (M_observed >= M_desired) and (N_observed >= N_desired):
    ...
    def lineplot(self, **kwargs) -> None:
        ...
        for s in np.arange(self.param.get('n_seq')):
            sequences.sel(sequence=(s+1)).plot(ax=ax, c=kwargs.pop('c', 'blue'), linewidth=kwargs.pop('linewidth', 0.25))
