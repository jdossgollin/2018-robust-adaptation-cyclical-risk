...
def get_gen_fun(fun_name, N, M, t0, n_seq):
    ...
    if fun_name == 'NINO3':
        ...
        gen_fun = TwoStateSymmetricMarkovLN2(
            ...
            coeff_var = 0.1, sigma_min = 0.01,
            model_name='TwoStateSymmetricMarkovLN2_Trend'
        )
    else:
...
def get_fit_fun(fun_name, gen_fun, n_sim):
    ...
    if fun_name == 'LN2 Stationary':
        fit_fun = StationaryLN2Stan(sflow=gen_fun, n_sim=n_sim, chains=1, warmup=1000, model_name='StationaryLN2Stan_Trend')
    elif fun_name == 'LN2 Trend':
        fit_fun = TrendLN2Stan(sflow=gen_fun, n_sim=n_sim, chains=1, warmup=1000, model_name='TrendLN2Stan_Trend')
    elif fun_name == 'HMM':
        fit_fun = HMM(sflow=gen_fun, n_sim=n_sim, n_components=2, n_init=50, model_name='HMM_Trend')
    else:
        raise ValueError('Invalid Index')
    return fit_fun
